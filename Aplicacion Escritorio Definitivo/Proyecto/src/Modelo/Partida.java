...
public class Partida {
    ...
    public Partida(String nombreJugador, String d) {
        this(nombreJugador, tipo_partida.desafio, d);
    }
    ...
    public Partida(String nombreJugador, boolean rapida, String d) {
    	jugador = new Jugador(nombreJugador, rapida ? vidas[1] : vidas[0]);
        if (!rapida) tablero = new Tablero();
        else tablero = new Tablero(d);
        dado = new Dado();
        ficha = new Ficha();
    }
    ...
    public void avanzarFicha(int n) {
        int cont = 0;
        while (ficha.getPosicion() < tablero.getCasillas().size() && cont++ < n) ficha.avanzar(1);
